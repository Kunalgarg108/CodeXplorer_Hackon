...
export default function CreateIncomes({ refreshData }) {
  ...
  const onCreateIncomes = async () => {
    await api.createIncome({ name, amount, icon: emojiIcon });
    refreshData();
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            <div className="mt-5">
              ...
              <div className="mt-4">
                <label className="text-fog text-sm font-thin block mb-1">Source Name</label>
                <Input placeholder="e.g. Youtube" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
              <div className="mt-4">
                <label className="text-fog text-sm font-thin block mb-1">Monthly Amount</label>
        ...
        <DialogFooter className="sm:justify-start">
          <DialogClose asChild>
            <Button disabled={!(name && amount)} onClick={onCreateIncomes} className="mt-5 w-full">
